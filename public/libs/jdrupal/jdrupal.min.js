...
jDrupal.connect=function(){
    return new Promise(function(a,b){
        ...
        c.open("GET",jDrupal.restPath()+"jdrupal/connect?_format=json");
        var d=function(){
            jDrupal.connected=!0;
            var b=JSON.parse("undefined"!=typeof c.responseText?c.responseText:c.response);
            0==b.uid?(jDrupal.setCurrentUser(jDrupal.userDefaults()),
            a(b)):jDrupal.userLoad(b.uid).then(function(c){
                jDrupal.setCurrentUser(c),a(b)})
        };
        c.onload=function(){
            console.log('connect request: ', c);
            if(200!=c.status) return void b(Error(c.statusText));
            var a=jDrupal.moduleInvokeAll("rest_post_process",c);
            a?a.then(d):d()
        },
        c.onerror=function(){
